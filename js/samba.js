...
function Events1() { // samba 
	...
	function on_page_loaded(task) {
		...
		if (!task.safe_mode || task.user_info.role_id == 1) {	
			...
			$("#taskmenu").hide();
			task.samba_shares.view($("#content"));	
		}
...
function Events7() { // samba.journals.samba_shares 

	function on_view_form_created(item) {
		var height = $(window).height() - $('body').height(),
			samba_server,
			time_out;
		item.samba_server = task.samba_server.copy({handlers: false});
		item.samba_server.set_fields(['id', 'server_name']);
		// item.samba_server.paginate = true;
		item.samba_server.view_options.fields = ['server_name'];
		item.samba_server.create_table(item.view_form.find(".view-server"), {
			height: height,
			dblclick_edit: false
		});
		item.samba_server.open({open_empty: true});	
		samba_server = task.server('get_user_samba_server');
		for (var i = 0; i < samba_server.length; i++) {
			item.samba_server.append();
			item.samba_server.id.value = samba_server[i][0];
			item.samba_server.server_name.value = samba_server[i][1];
			item.samba_server.post();
		}
		item.samba_server.first();
		item.samba_server.on_after_scroll = function(al) {
		...
		item.samba_server.on_after_scroll(item.samba_server);
		
		item.create_table(item.view_form.find(".view-table"), {height: height});
		item.add_button(item.view_form.find(".form-footer"), 'Save').click(function() {
			item.server('save_to_file', [item.share_path.value]);
		});
	}
	
	function on_before_post(item) {
		item.added_date.value = new Date();
		item.share_path.value = item.samba_server.id.value;
	}
	this.on_view_form_created = on_view_form_created;
